...
import { UserData, UserGender, eighteenYears, InfoModalData } from '../../../globalTypes';
...
const MainPage: React.FC = () => {
    ...
    const [isError, setIsError] = useState<Record<keyof Omit<UserData, "name" | "gender">, boolean>>({
        surname: false,
        email: false,
        birthDate: false,
      });
        ...
        (
            async () => {
                try {
                    ...
                    // const res: AxiosResponse = await axios.get('https://extensi.io/api/email-validator.php',
                    // {
                    //     params: {
    ...
    const sendDataHandling = async () => {
        ...
        try {
            setIsAdding(true);
            await new Promise(resolve => setTimeout(resolve, 2000));
            const res: AxiosResponse = await axios.post('http://localhost:3005/user' , userDataToSend , {
                headers: {
            ...
            setModal({
                ...
                title: 'Error',
                content: {
                    errorContent: err.response.data.message ? err.response.data.message : 'Something went wrong'
                }
            })
            ...
            <Paper elevation={3} className={classes.root}>
                <Grid container justifyContent="center">
                    <Grid item xs={12} sm={8} lg={6} className={classes.dataForm} >
                        <Typography variant="h6" style={{width: "fit-content"}}>Enter your data</Typography>
                        <CustomInput
                            labelText="Name"
                            helperText=""
                            id="name"
                        ...
                        <CustomInput
                            ...
                            helperText={isError.birthDate ? 'you must be 18 years or older' : ''}
                            id="birthDate"
                        ...
                        <RadioGroup
                            ...
                            />
                            <FormControlLabel
                                control={<Switch disabled={isAdding} checked={isGender} onChange={switchIsEditHandling} />}
                                label="Set gender"
                            />
                        </RadioGroup>
